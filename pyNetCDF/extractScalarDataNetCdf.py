...
x, x_dims = read1DVariableFromDataset('x', sN, ds, 0, 1, cl ) # Scalar grid's x, exclude the last value.
xv = createNetcdfVariable( dso, x, 'x', len(x), 'm', 'f4', ('x',), parameter )
x = None

y, y_dims = read1DVariableFromDataset('y', sN, ds, 0, 1, cl ) # Scalar grid's y, exclude the last value.
yv = createNetcdfVariable( dso, y, 'y', len(y), 'm', 'f4', ('y',), parameter )
y = None

z, z_dims = read1DVariableFromDataset('z', sN, ds, 1, 0, cl ) # Scalar grid's z, exclude the first value.
