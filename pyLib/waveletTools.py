...
#==========================================================#
  def SigMorletScalogram(self,ttype="complex", plot=True):
    """
    ...
    if(ttype=="real"):
      ...
      if(plot==True):
        ...
        fig3.suptitle("RealMorlet wavelet signal scalogram",fontsize=16) 
        cax=ax3a.imshow(output, extent=[self.t[0], self.t[-1], self.scales[-1], self.scales[0] ], cmap='PRGn',\
      ...
      if plot==True:
        ...
        cax=ax3a.imshow(np.real(output), extent=[self.t[0], self.t[-1], self.scales[-1], self.scales[0] ], \
        ...
        cax=ax3b.imshow(np.imag(output), extent=[self.t[0], self.t[-1], self.scales[-1], self.scales[0] ],\
          cmap='PRGn', aspect='auto', vmax=abs(output).max(), vmin=-abs(output).max())
        plt.colorbar(cax)
...
#==========================================================#
  ...
  def SigMorletSpectrogram(self,ttype="complex",plot=True):
    """
    RealMorlet wavelet transform of the signal  with respect to frequency, taking into account the set central frequency.
    Arguments:
    ttype (optional): it's the string "complex" by default; it can be set to "real"
    ...
    for ind, width in enumerate(1./self.freq*self.omega0/(2*np.pi)):
        wavelet_data = Morlet(min(width*10./dt,len(self.data)), width,self.omega0,dt)
        output[ind, :] = 1./np.sqrt(width)*dt*signal.convolve(self.data, np.real(wavelet_data),
                                  mode='same')+1./np.sqrt(width)*dt*signal.convolve(self.data, np.imag(wavelet_data),
                                  mode='same')*1j
    ...
    if ttype=="real":
      ...
      if plot==True:
        ...
        cax=ax3a.imshow(output, extent=[self.t[0], self.t[-1], self.freq[-1], self.freq[0] ], cmap='PRGn', aspect='auto',
      ...
      if plot==True:
        ...
        ax3a=fig3.add_subplot(2,1,1)
        ax3a.set_title("Real Part")
        cax=ax3a.imshow(np.real(output), extent=[self.t[0], self.t[-1], self.freq[-1], self.freq[0] ], cmap='PRGn', aspect='auto',
            vmax=abs(output).max(), vmin=-abs(output).max())
        plt.colorbar(cax)
        ...
        ax3a.set_ylabel('frequency (Hz)')
        ax3b.set_title("Imaginary Part")
        cax=ax3b.imshow(np.imag(output), extent=[self.t[0], self.t[-1], self.freq[-1], self.freq[0] ], cmap='PRGn', aspect='auto',
            vmax=abs(output).max(), vmin=-abs(output).max())
        plt.colorbar(cax)
        ax3b.set_xlabel('time (s)')
...
#==========================================================#
  def PowerMorletScalogram(self,ttype="complex"):
    """
    ...
    output=abs(self.SigMorletScalogram(ttype=ttype,plot=False))**2
    fig3=plt.figure()
    ax3a=fig3.add_subplot(1,1,1)
...
#==========================================================#
  def PowerMorletSpectrogram(self,ttype="complex"):
    """
    square modulus of the wavelet transform of the signal: it's the power of every component
    Arguments:
    ttype (optional): it's the string "complex" by default; it can be set to "real"
...
#==========================================================#
  def MorletHisto(self,ix, mode="frequency",nbins=30):
    ...
    fig=plt.figure()
    plt.title("Histogram of fluctuations at {} = {} ".format(mode,self.freq[ix]) )
    if mode=="frequency":
      CWT=self.SigMorletSpectrogram(ttype="real",plot=False)
      plt.hist(CWT[ self.freq[ix] ,:],nbins)
      #   print str(list(self.freq))
      #   print str(CWT[ :,(list(self.freq)).index(par)])
    if mode=="scale":
      CWT=self.SigMorletScalogram(ttype="real",plot=False)
      plt.hist(CWT[ self.scales[ix],:],nbins)
