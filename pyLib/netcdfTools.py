...
def createCoordinateAxis(dso, Rdims, Rdpx, axis, varname, formatstr, unit, parameter):
    arr = np.empty(Rdims[axis]);
    for i in xrange(Rdims[axis]):
        arr[i] = i*Rdpx[axis] # dpx is in [N,E], see getGeoTransform() in gdalTools.py
    axvar = createNetcdfVariable(dso, arr, varname, len(arr), varname, formatstr, (varname,), parameter)
    arr = None
    return axvar
